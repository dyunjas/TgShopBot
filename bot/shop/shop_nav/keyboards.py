...
async def build_shop_keyboard(*, shop_id: int, shop_repo: ShopRepository, parent_id: int | None = None):
    builder = InlineKeyboardBuilder()

    categories = await shop_repo.get_categories(shop_id=shop_id, parent_id=parent_id)
    ...
    if parent_id is not None:
        items = await shop_repo.get_items_by_category(shop_id=shop_id, category_id=parent_id)
        for item in items:
            builder.button(
                text=f"{item.title}",
    ...
    builder.adjust(ORDERS_PER_ROW)

    if parent_id is None:
        builder.row(InlineKeyboardButton(text="Назад", callback_data="main_menu"))
    else:
        current_category = await shop_repo.get_category_by_id(shop_id=shop_id, category_id=parent_id)
