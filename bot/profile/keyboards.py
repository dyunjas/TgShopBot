...
def build_orders_kb(orders, page: int = 0):
    ...
    for order in chunk:
        builder.button(text=f"📦{order.title} [{order.order_id}]", callback_data=f"order:{order.id}:{page}")
    builder.adjust(1)
...
def build_transactions_kb(transactions, page: int = 0):
    ...
    for transaction in chunk:
        builder.button(text=f"💳 {transaction.amount} RUB", callback_data=f"transaction:{transaction.id}:{page}")
    builder.adjust(1)
...
def back_to_transactions_bt(page):
    builder = InlineKeyboardBuilder()
